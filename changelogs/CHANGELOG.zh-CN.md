...
> **v2.1.5** (2026-08-13) **[新增]** 关于页 CLI 手册弹窗、CLI 一键加入/移出 PATH 脚本、Scoop 安装渠道识别（含 CLI/仅 GUI 标注）；**[优化]** CLI 更新可靠性提升 — 检查与下载失败自动重试 3 次、下载断点续传 + 实时进度条 + SHA256 校验、后台更新脚本防中文路径乱码、`lpb info` 新增安装目录、更新检查状态行统一、CLI 命令别名精简至 4 个（移除 lipbox / lpbx）；**[修复]** 更新检查把正式版误判为预览版（4 段 vs 3 段版本号比较修正）
>
