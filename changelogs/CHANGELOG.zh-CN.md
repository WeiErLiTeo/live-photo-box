...
> **v2.0.0** (2026-07-18) **[重大发布]** **新增实况照片编辑（LivePhotoEdit）页面** — 时间轴（自动提取视频帧、逐帧选取）、导出单帧或全部帧、封面替换、属性查看、文件拖拽；**所有页面支持文件夹拖拽**；**时间轴模式切换**（经典/胶片）；**日志查看增强**（本次+上一次日志）；**修复 v1.15.2/v1.15.3 安装包工具缺失问题**；**修复便携版/安装版语言切换无效**（自最初版本存在的历史问题）

## v1.15.x — 自动更新与配对增强

> **v1.15.3** (2026-07-03) **[修复+优化]** 修复修复页扫描后空队列提示不消失（三个 ViewModel 共补 6 处 UpdateIsQueueEmpty 调用）；**设置页跳转定位修复** — 改用 ChangeView 替代 BringIntoView，每次跳转准确显示目标标题；**启动首屏动画移除**（侧栏切换动画保留）；空队列提示增加图标引导小字；空队列提示区域垂直居中优化；修复 HistoryPage XAML Color 类型警告
>
> **v1.15.2** (2026-07-03) **[新增+修复+优化]** 修复选项对话框；灯箱全面升级（Live Photo 播放、智能翻页动画、视频播控栏）；修复非打包模式 GitHub Token 保存不持久的问题；**安装包体积骤降 80% — FFmpeg 从 97 MB 通用编译定制瘦身至 5.8 MB**（精确保留 x264/x265/AMF/NVENC/QSV 硬件编码 + 缩略图 + 视频修复旋转，移除上千个无用组件）；**修复页"已跳过"细分具体原因**（非Apple / HEIC/HEIF / 时长>3.5s / 无需处理）；**诊断结果不再被跳过标记覆盖**；**修复选项弹窗统计不含已跳过文件**；**修复关闭窗口访问违例崩溃**；**队列一键回到顶部悬浮按钮**
>
