...
`LivePhotoBox.Core/Services/Protocols/LivePhotoProtocol.cs` → `WrapXmp()` 方法
...
`LivePhotoBox.Core/Services/LivePhotoSplitService.cs` → `SplitAsync()` 方法末尾（第 184-187 行）
...
`LivePhotoBox.Core/Services/LivePhotoRepairService.cs`
...
| 功能 | 文件 | 关键方法 |
|------|------|---------|
| Combo 标记注入 | `LivePhotoBox.Core/Services/Protocols/LivePhotoProtocol.cs` | `WrapXmp()` |
| Split 标记写入 | `LivePhotoBox.Core/Services/LivePhotoSplitService.cs` | `SplitAsync()` 第 184-187 行 |
| Repair 标记写入 | `LivePhotoBox.Core/Services/LivePhotoRepairService.cs` | `TryWriteLivePhotoBoxMarkerAsync()` |
| ExifTool 快捷写入 | `LivePhotoBox.Core/Services/LivePhotoRepairService.cs` | `RunExifToolAsync()` |
| 历史记录解析 | `LivePhotoBox/ViewModels/HistoryViewModel.cs` | `ParseXmp()`, `ParseHistorySubject()` |
| 历史记录数据模型 | `LivePhotoBox/Models/FileHistoryInfo.cs` | `FileHistoryInfo`, `HistoryEntry` |
| 历史页面 UI | `LivePhotoBox/Views/HistoryPage.xaml` | - |
| 历史页面逻辑 | `LivePhotoBox/Views/HistoryPage.xaml.cs` | `SelectFolder_Click()` |
