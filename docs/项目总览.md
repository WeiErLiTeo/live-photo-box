...
│   ├── release-v*.md                 # 各版本发布说明（v1.14~v2.1）
│   └── microsoft-store/              # Microsoft Store 商店更新日志
│
├── docs/                             # 项目文档
│   ├── CLI-User-Guide-en.md          # CLI 使用指南（英文）
│   ├── CLI-使用指南-zh-CN.md         # CLI 使用指南（中文）
...
│           └── HuaweiMovingPhotoProtocol.cs / HuaweiMovingPhotoV3Protocol.cs
│
├── LivePhotoBox.CLI/             # 📦 命令行工具（livephotobox）
...
│   │   ├── ProtocolTestExporter.cs  # 协议测试导出
...
└── HuaweiMovingPhotoProtocol   (Id=6) — HUAWEI（JPEG/HEIC + MP4 尾部）
      支持 HEIC + H.265 (HEVC) 输出与无损 HEVC 直通；使用者: 华为/荣耀
      另有 HuaweiMovingPhotoV3Protocol（HarmonyOS 4.0 V3，定义但暂未注册）
```
...
|------|------|
| 项目 | 3 (Core + GUI + CLI) |
| C# 源文件 (.cs) | ~128（Core 42 + GUI 79 + CLI 7） |
| XAML 页面/控件 (.xaml) | 14 |
...
| Protocols | 9（1 抽象基类 + 7 注册实现 + 1 V3 未注册） |
| Converters | 7 |
