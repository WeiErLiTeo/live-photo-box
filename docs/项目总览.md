...
| 功能 | 说明 |
|------|------|
| **🔗 实况照片合成 (Combo)** | 将任意静态图片 + 视频素材组合为标准实况照片。支持 `Micro Video` / `Motion Photo` / OPPO / vivo / Samsung / HUAWEI 等协议，自动写入完整 `EXIF` + `QuickTime` 元数据 |
...
│       └── Protocols/               # 实况照片协议（1 抽象基类 + 6 注册实现）
...
| 目标平台 | x64 (win-x64) |
| 支持的实况协议 | 6 (Micro Video / Motion Photo / OPPO / vivo / Samsung / HUAWEI) |
