...
- `LivePhotoBox.CLI/LivePhotoBox.CLI.csproj` — 命令行工具（`AssemblyName=livephotobox`，`net9.0-windows10.0.19041.0`，`PlatformTarget=x64`）；构建后为每个别名复制可执行文件（`livephotobox` / `livephoto` / `livebox` / `lipbox` / `lpb` / `lpbx`）
