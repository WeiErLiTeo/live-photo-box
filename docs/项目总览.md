...
| 功能 | 说明 |
|------|------|
| **🔗 实况照片合成 (Combo)** | 将任意静态图片 + 视频素材组合为标准实况照片。支持 Fusion / `Micro Video V1` / `Motion Photo V2` / OPPO / vivo / Samsung / HUAWEI 等协议，自动写入完整 `EXIF` + `QuickTime` 元数据 |
...
│   ├── Program.cs                  # 入口（System.CommandLine，merge/protocols/update-check）
...
├── MotionPhotoFusionProtocol   (Id=0) — Fusion（V2 + OPPO + vivo + Samsung 元数据融合）
...
├── VivoLivePhotoProtocol       (Id=4) — vivo（扩展 V2 + VCamera 命名空间）
│     使用者: vivo X300 系列及之后
├── SamsungMotionPhotoProtocol  (Id=5) — Samsung（扩展 V2 + SEF 尾部）
...
| 目标平台 | x64 (win-x64) |
| 支持的实况协议 | 7 (Fusion / V1 / V2 / OPPO / vivo / Samsung / HUAWEI) |
